<template>
  <!--begin::Menu-->
  <div
    class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold py-4 fs-6 w-275px"
    data-kt-menu="true"
  >
    <!--begin::Menu item-->
    <div class="menu-item px-3">
      <div class="menu-content d-flex align-items-center px-3">
        <!--begin::Avatar-->
        <div class="symbol symbol-50px me-5">
          <img alt="Logo" :src="getAssetPath('media/images/blank.png')" />
        </div>
        <!--end::Avatar-->

        <!--begin::Username-->
        <div class="d-flex flex-column">
          <div class="fw-bold d-flex align-items-center fs-5">
            {{ userLogged?.first_name }}
            <span class="badge badge-light-success fw-bold fs-8 px-2 py-1 ms-2">
              {{ userLogged?.user_class }}
            </span>
          </div>
          <a href="#" class="fw-semibold text-muted text-hover-primary fs-9">
            {{ userLogged?.email }}
          </a>
        </div>
        <!--end::Username-->
        
      </div>
    </div>
    <!--end::Menu item-->

    <!--begin::Menu separator-->
    <div v-show="Object.keys(userLogged).length > 0" class="separator my-2"></div>
    <!--end::Menu separator-->

    <!--begin::Menu item-->
    <div v-show="Object.keys(userLogged).length > 0" class="menu-item px-5">
      <router-link to="/pages/profile/overview" class="menu-link px-5">
        My Profile
      </router-link>
    </div>
    <!--end::Menu item-->

    <!--begin::Menu separator-->
    <div v-show="Object.keys(userLogged).length > 0" class="separator my-2"></div>
    <!--end::Menu separator-->

    <!--begin::Menu item-->
    <div
      class="menu-item px-5"
      data-kt-menu-trigger="hover"
      data-kt-menu-placement="left-start"
      data-kt-menu-flip="center, top"
    >
      <router-link to="/pages/profile/overview" class="menu-link px-5">
        <span class="menu-title position-relative">
          Language
          <span
            class="fs-8 rounded bg-light px-3 py-2 position-absolute translate-middle-y top-50 end-0"
          >
            {{ currentLangugeLocale.name }}
            <img
              class="w-15px h-15px rounded-1 ms-2"
              :src="currentLangugeLocale.flag"
              alt="metronic"
            />
          </span>
        </span>
      </router-link>

      <!--begin::Menu sub-->
      <div class="menu-sub menu-sub-dropdown w-175px py-4">
        <!--begin::Menu item-->
        <div class="menu-item px-3">
          <a
            @click="setLang('en')"
            href="#"
            class="menu-link d-flex px-5"
            :class="{ active: currentLanguage === 'en' }"
          >
            <span class="symbol symbol-20px me-4">
              <img
                class="rounded-1"
                :src="getAssetPath('media/flags/united-states.svg')"
                alt="metronic"
              />
            </span>
            English
          </a>
        </div>
        <!--end::Menu item-->

        <!--begin::Menu item-->
        <div class="menu-item px-3">
          <a
            @click="setLang('ph')"
            href="#"
            class="menu-link d-flex px-5"
            :class="{ active: currentLanguage === 'es' }"
          >
            <span class="symbol symbol-20px me-4">
              <img
                class="rounded-1"
                :src="getAssetPath('media/flags/philippines.svg')"
                alt="metronic"
              />
            </span>
            Philippines
          </a>
        </div>
        <!--end::Menu item-->


  
      </div>
      <!--end::Menu sub-->
    </div>
    <!--end::Menu item-->

    <!--begin::Menu item-->
    <div v-show="Object.keys(userLogged).length > 0" class="menu-item px-5 my-1">
      <router-link to="/pages/profile/overview" class="menu-link px-5">
        Account Settings
      </router-link>
    </div>
    <!--end::Menu item-->

    <!--begin::Menu item-->
    <div class="menu-item px-5">
      <a @click="signOut()" class="menu-link px-5">
        <span v-if="Object.keys(userLogged).length > 0"> Sign Out </span>
        <span v-else> Sign In </span>
      </a>
    </div>
    <!--end::Menu item-->
  </div>
  <!--end::Menu-->
</template>

<script lang="ts">
import { getAssetPath } from "@/assets/template/core/helpers/assets";
import { computed, defineComponent, ref, Component } from "vue";
import { useI18n } from "vue-i18n";
import { useAuthStore } from "@/assets/template/stores/auth";
import { useRouter } from "vue-router";
import { useCookies } from "vue3-cookies";

export default defineComponent({
  name: "kt-user-menu",
  components: {},
  props: {
    data: { type: Object, required: true, default: {} },
  },
  setup(props) {
    const router = useRouter();
    const i18n = useI18n();
    const store = useAuthStore();
    const userLogged = ref(props.data);
    const { cookies } = useCookies();

    i18n.locale.value = localStorage.getItem("lang")
      ? (localStorage.getItem("lang") as string)
      : "en";

    const countries = {
      en: {
        flag: getAssetPath("media/flags/united-states.svg"),
        name: "English",
      },
      ph: {
        flag: getAssetPath("media/flags/philippines.svg"),
        name: "Philippines",
      },
    };

    const signOut = () => {
      cookies.remove("userId");
      cookies.remove("sectionId");
      store.logout();
      router.push({ name: "signIn" });
    };

    const setLang = (lang: string) => {
      localStorage.setItem("lang", lang);
      i18n.locale.value = lang;
    };

    const currentLanguage = computed(() => {
      return i18n.locale.value;
    });

    const getCookieSectionId = computed(() => {
      return cookies.get("sectionId");
    });

    const currentLangugeLocale = computed(() => {
      return countries[i18n.locale.value as keyof typeof countries];
    });

    return {
      signOut,
      setLang,
      currentLanguage,
      currentLangugeLocale,
      countries,
      getAssetPath,
      userLogged,
      getCookieSectionId,
    };
  },
});
</script>
