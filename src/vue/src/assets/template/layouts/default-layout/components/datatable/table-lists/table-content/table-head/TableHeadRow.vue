    ...
    <tr class="text-start text-gray-500 fw-bold fs-7 text-uppercase gs-0">
      <th v-if="checkboxEnabled" :style="{ width: '30px' }">
        <div class="form-check form-check-sm form-check-custom form-check-solid me-3 ps-3">
          <input class="form-check-input border border-gray-400 cursor-pointer" type="checkbox" v-model="checked"
            @change="selectAll()" />
        </div>
      </th>
      <th v-if="rowNumberingEnabled" :style="{ width: '30px' }">
        <div class="form-check form-check-sm form-check-custom form-check-solid text-center">#</div>
      </th>
      <template v-for="(column, i) in header" :key="i">
        <th :class="`${(i === header.length - 1) ? 'border-start border-gray-300' : ''} ${column.labelClass}`"
...
import type { Sort } from "@/components/kt-datatable/table-lists/models";
