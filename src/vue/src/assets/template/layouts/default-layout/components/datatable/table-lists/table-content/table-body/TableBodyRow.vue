<template>
  <tbody class="text-gray-900">
    <template v-for="(row, i) in data" :key="i">
      <tr :class="onReinitialize(i, data.length)">
        <td v-if="checkboxEnabled" class="bg-light">
          <div class="form-check form-check-sm form-check-custom form-check-solid ps-3">
            <input class="form-check-input border border-gray-400 cursor-pointer" type="checkbox"
              :value="row[checkboxLabel]" v-model="selectedItems" @change="onChange" />
          </div>
        </td>
        <td v-if="rowNumberingEnabled" class="bg-light text-center">
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const onReinitialize = (index, totalLength) => {
