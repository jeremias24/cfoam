        ...
        `requests/${MixinService.encrypt(
          JSON.stringify({
            ...
            filter: {
              ...
              "ur.userRequestId": initData.value?.referenceId,
            },
          })
        )}`
      )
          ...
          setTimeout(() => {
            if (data?.userRequests.length) {
              const approvals = data.userRequests[0].approvals.filter(
                (value) => value.referenceId == initData.value?.referenceId
              );

              if (data.userRequests[0]?.statusDescription == "Completed") {
                if (
                ...
                data.userRequests[0]?.statusDescription == "Rejected"
              ) {
                const rejectedApproval = approvals.filter(
                  (value) => value.statusDescription == "Rejected"
                );
                ...
                data.userRequests[0]?.statusDescription == "Cancelled"
              ) {
                initAction.value = "Already Cancelled";
              } else if (
                initData.value?.action == "Confirm" &&
                data.userRequests[0]?.statusDescription == "Submitted"
              ) {
                if (approvals.length) {
                  const pendingApprovals = approvals.filter(
                    (value) => value.statusDescription == "Pending"
                  );
