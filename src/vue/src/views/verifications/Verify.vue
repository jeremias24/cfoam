...
import Loading from "@/components/forms/verifications/Loading.vue";
  ...
  setup() {
    ...
    onMounted(() => {
      try {
        ...
        fetchVerifyData(initData.value);
      } catch (error) {
        router.push("/error/404");
      }
    });

    const fetchVerifyData = (data) => {
      ApiService.put(
        ...
        .then(({ data }) => {
          setTimeout(() => {
            if (data.status == "Verified") {
              Object.assign(initData.value, { status: data.status });
